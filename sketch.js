...
function preload() {
    ...
    bglevel1img=loadImage("images/bglevel1.gif")
...
function Level2() {
    ...
    background("black")
    if (frameCount % 60 === 0) {
        taps = createSprite(width, 20)
        taps.y = Math.round(random(10, 40))
        taps.x = Math.round(random(width, 20))

        taps.velocityX = -2
        taps.velocityY = Math.round(random(2, 6))
        taps.addImage(runningTap)
        taps.scale = .1
